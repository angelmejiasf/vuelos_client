...
class VuelosService {
    ...
    public function request_curlONE($identificador) {
        $urlmiservicio = "http://localhost/_serWeb/serVuelos/Vuelos.php?identificador=" .$identificador;
        $conexion = curl_init();
        curl_setopt($conexion, CURLOPT_URL, $urlmiservicio);
        curl_setopt($conexion, CURLOPT_HTTPGET, TRUE);
        curl_setopt($conexion, CURLOPT_RETURNTRANSFER, true);
